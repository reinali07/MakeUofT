...
HIAI_IMPL_ENGINE_PROCESS("SaveFilePostProcess_1", SaveFilePostProcess_1, INPUT_SIZE)
{
    ...
    for(unsigned int ind = 0; ind < tran->b_info.batch_size; ind++)
    {
        ...
        for(unsigned int i=0 ; i < output_data_vec.size(); ++i){
            ...
            GetOutputName(name);
            std::string outFileName = prefix + "_" + name + ".png";
            //int fd = open(outFileName.c_str(), O_CREAT| O_WRONLY, FIlE_PERMISSION);
            cv::Mat outMat (388,388,CV_32SC1,result); //read output vector to cv::mat
            outMat = outMat*100;
            /*std::string img_infor_tfilename = store_path + "/" + img_infor.tfilename;
            std::string output_file = "result_files/0/" + img_infor.tfilename; //set output file name
            cv::Mat outMat (1,size,CV_32SC1,result); //read output vector to cv:mat
            */double minVal,maxVal;
            cv::Point min_loc,max_loc;
            cv::minMaxLoc(outMat,&minVal,&maxVal,&min_loc,&max_loc); //find max prob, and index*/
            cv::imwrite(outFileName, outMat);
            /*std::string output_string ="Class: "+to_string(max_loc.x)+" Prob: " + to_string(maxVal);
            int fd1 = open(output_file.c_str(), O_CREAT| O_WRONLY|O_APPEND, S_IRUSR|S_IWUSR);
            int ret1 = write(fd1, output_string.c_str(), output_string.length()); 
            ret1 = close(fd1);*/
